...
def draw_scene_assets_list(layout: UILayout, context: Context, props) -> int:
    ...
    if total_count == 0:
        if asset_list.export_assets_refresh_pending() and not props.anim_usd_export_list_locked:
            ...
            alert.alert = True
            alert.label(
                text="No export targets — link *_publish* rig, add cameras, or Add Selected",
                icon="ERROR",
            )
    else:
    ...
    list_ops.operator(
        "wm.mono_fx_anim_usd_export_list_add_selected",
        text="Add Selected",
        icon="ADD",
    )
    list_ops.operator(
        "wm.mono_fx_anim_usd_export_list_remove",
        text="Remove All",
        icon="TRASH",
    )
